...
yq e --inplace ". | select(.kind == \"DaemonSet\" and .metadata.name == \"speaker\").spec.template.spec.volumes += {\"name\": \"{{ if .IsOpenShift }}\"}" ${FRR_MANIFESTS_DIR}/${FRR_MANIFESTS_FILE}
...
yq e --inplace ". | select(.kind == \"Deployment\" and .metadata.name == \"controller\").spec.template.spec.volumes += [{\"name\": \"{{ if .IsOpenShift }}\"}]" ${FRR_MANIFESTS_DIR}/${FRR_MANIFESTS_FILE}
...
yq e --inplace ". | select(.kind == \"DaemonSet\" and .metadata.name == \"speaker\").spec.template.spec.containers += {\"name\": \"{{ if .IsOpenShift }}\"}" ${FRR_MANIFESTS_DIR}/${FRR_MANIFESTS_FILE}
...
yq e --inplace ". | select(.kind == \"Deployment\" and .metadata.name == \"controller\").spec.template.spec.containers += {\"name\": \"{{ if .IsOpenShift }}\"}" ${FRR_MANIFESTS_DIR}/${FRR_MANIFESTS_FILE}
...
sed -i "s/- name: '{{ if .IsOpenShift }}'/{{ if .IsOpenShift }}/g" ${FRR_MANIFESTS_DIR}/${FRR_MANIFESTS_FILE}
