...
FRRK8S_VERSION="main" # TODO: remove
#FRRK8S_VERSION=v$(yq e '.dependencies[] | select(.name == "frr-k8s") | .version'  ${METALLB_PATH}/charts/metallb/Chart.yaml)
FRRK8S_MANIFESTS_FILE="frr-k8s.yaml"
FRRK8S_MANIFESTS_URL="https://raw.githubusercontent.com/metallb/frr-k8s/${FRRK8S_VERSION}/config/all-in-one/${FRRK8S_MANIFESTS_FILE}"
curl ${FRRK8S_MANIFESTS_URL} -o _cache/${FRRK8S_MANIFESTS_FILE}
# Generate frr-k8s rbac manifests
sed -i -e 's/frr-k8s-system/metallb-system/g' "_cache/${FRRK8S_MANIFESTS_FILE}"
yq e '. | select(.kind == "Role" or .kind == "ClusterRole" or .kind == "RoleBinding" or .kind == "ClusterRoleBinding" or .kind == "ServiceAccount")' _cache/${FRRK8S_MANIFESTS_FILE} > config/metallb_rbac/${FRRK8S_MANIFESTS_FILE}
...
#sed -i -e 's/metallb-webhook-service/webhook-service/g' "$FRRK8S_PATH"/charts/metallb/templates/controller.yaml
